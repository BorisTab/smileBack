 ...
 */
int writeFile(const char outPath[], char *text, size_t textSize) {
    assert(outPath != nullptr);
    assert(text != nullptr);

    FILE *myFile = fopen(outPath, "w");
    if (!myFile) {
        perror("File opening failed");
        return errno;
    }

    fwrite(text, textSize, 1, myFile);
...
char *readTextFromFile(const char inPath[], size_t *textSize) {
    assert(inPath != nullptr);
